// index.js — AlphaStream v27.0 — LOW FLOAT PENNY MONSTER + HARD RISK (2025)
...
async function exitAt345OrLoss() {
  ...
  if (dailyPnL <= MAX_DAILY_LOSS) {
    if (Object.keys(positions).length > 0) {
      await log("LOSS_STOP", "SYSTEM", `Daily loss hit ${dailyPnL*100}% → closing all`);
...
async function scanLowFloatPennies() {
  ...
  try {
    ...
    try {
      ...
      await log("GAINERS_ERROR", "SYSTEM", "Using fallback snapshot");
    }

    // If gainers fail → fallback to full snapshot (still fast)
    ...
    for (const c of candidates) {
      ...
      const { Supertrend, ADX, ATR } = await import("technicalindicators");
      ...
      if (current.adx > 25 && current.stTrend === 1 && current.price > current.stLine) {
        ...
        if (prob > 0.80) {
          const riskAmount = 25000 * RISK_PER_TRADE;
          ...
          await log("LOW_FLOAT_ENTRY", c.symbol, `+${c.gap.toFixed(1)}% | Float ${(float/1e6).toFixed(1)}M | Risk 1%`, { qty, prob: (prob*100).toFixed(1) });
        }
      }
    }
...
app.listen(PORT, "0.0.0.0", async () => {
  console.log(`ALPHASTREAM v27.0 LOW FLOAT PENNY LIVE`);

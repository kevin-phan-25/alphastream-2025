...
async function logToGAS(event, symbol = "", note = "", data = {}) {
  if (!LOG_WEBHOOK_URL)
    return console.log(`[LOG] ${event} | ${symbol} | ${note}`, data);
...
async function getEquity() {
  const r = await safeGet(`${ALPACA_BASE}/account`, {
...
async function getEliteGappers(limit = 1000) {
  const data = await safeGet(
    `${MASSIVE_BASE}/v3/reference/tickers?market=stocks&active=true&limit=${limit}&apiKey=${MASSIVE_KEY}`
  );
...
async function analyzeCandidate(t) {
  try {
    ...
    const recentVol =
      bars.slice(-5).reduce((s, b) => s + (b.v || 0), 0) / 5;
    const avgVol =
      bars.slice(-30, -5).reduce((s, b) => s + (b.v || 0), 0) / 25 || 1;
    ...
    const qty = Math.max(
      1,
      Math.floor((equity * 0.015) / riskPerShare)
    );
    return {
      symbol: t.sym,
      price: last.c,
...
async function placeBracketOrder(sig) {
  const payload = {
    ...
    order_class: "bracket",
    take_profit: {
      limit_price: parseFloat((sig.price * 1.1).toFixed(2)),
    },
    stop_loss: {
      stop_price: parseFloat(
        (sig.price * (1 - (sig.atr ? sig.atr / sig.price : 0.04))).toFixed(
          2
        )
      ),
    },
  ...
  try {
    ...
    await logToGAS("ENTRY", sig.symbol, `BUY ${sig.qty} @ ${sig.price}`, {
      ml: sig.mlScore,
    });
    positions[sig.symbol] = {
      entry: sig.price,
      qty: sig.qty,
      trailPrice: sig.price * 0.94,
      partialDone: false,
    };
    return r.data;
  } catch (e) {
    ...
    for (const pos of open) {
      ...
      if (current >= entry * 1.05 && !positions[symbol].partialDone) {
        ...
        await logToGAS(
          "PARTIAL",
          symbol,
          `Locked +5% on ${Math.floor(qty * partialPct)} shares`
        );
      }
...
async function scanHandler() {
  ...
  try {
    ...
    if (Object.keys(positions).length >= MAX_POS) {
      isScanning = false;
      return;
    }
  ...
  try {
    ...
    const candidatePromises = gappers.map(async (t) => {
      ...
      await logToGAS(
        "PREMARKET_CANDIDATE",
        sig.symbol,
        `score=${sig.mlScore.toFixed(3)} atr=${sig.atr?.toFixed(4)}`,
        logData
      );
    ...
    await logToGAS(
      "PREMARKET_SUMMARY",
      "SYSTEM",
      `Found ${premarketCandidates.length} candidates`,
      { count: premarketCandidates.length }
    );
...
function scheduleDailyPremarket(hourUTC = 12, minuteUTC = 45) {
  ...
  setTimeout(async function runAndInterval() {
    try {
      ...
      console.error("scheduled premarket error", e.message);
    }
...
// -------------------- Routes --------------------
app.get("/", (req, res) =>
  res.json({ status: "AlphaStream v23.3", time: new Date().toISOString() })
);

app.post("/", async (req, res) => {
  const body = req.body || {};
  if (FORWARD_SECRET && body.secret !== FORWARD_SECRET)
